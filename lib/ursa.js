...
function PublicKey(rsa) {
  ...
  function toPublicPem(encoding) {
    return encodeBuffer(rsa.exportKey('pkcs8-public-pem') + "\n", encoding);
  }
  ...
  function publicDecrypt(buf, bufEncoding, outEncoding) {
    throw new Error("Unsupported operation : publicDecrypt");
    //buf = decodeString(buf, bufEncoding);
...
function createPrivateKey(pem, password, encoding) {
  ...
  try {
    ...
    if (password != null) {
      throw new Error("Unsupported method : createPrivateKey with password");
...
function createPrivateKeyFromComponents(modulus, exponent, p, q, dp, dq, inverseQ, d) {
  ...
  return PrivateKey(rsa);*/
  throw new Error("Unsupported method : createPrivateKeyFromComponents");
}
